...
describe("COLOR_SCHEME", () => {
  ...
  it("Tell color is pink (255,128,255)", () => {
    const tell = COLOR_SCHEME[12];
    expect(tell).toEqual(expect.objectContaining({ r: 255, g: 128, b: 255 }));
    expect(tell?.name).toBe("Tell");
  });

  it("Guild color is green (0,230,0)", () => {
    const guild = COLOR_SCHEME[11];
    expect(guild).toEqual(expect.objectContaining({ r: 0, g: 230, b: 0 }));
    expect(guild?.name).toBe("Guild");
  });

  it("Default color is white (255,255,255)", () => {
    const defaultColor = COLOR_SCHEME[1];
    expect(defaultColor).toEqual(
      expect.objectContaining({ r: 255, g: 255, b: 255 }),
    );
    expect(defaultColor?.name).toBe("Default");
  });
