...
set_wine_resolution() {
    ...
    nn_log "Setting Wine virtual desktop to ${res}..."

    if [[ "${DRY_RUN}" -eq 1 ]]; then
        nn_log "  [DRY-RUN] Would set desktop to ${width}x${height}"
        return 0
    fi
    ...
    nn_log "  Wine virtual desktop: ${res}"
}
...
set_eq_resolution() {
    ...
    if [[ ! -f "${ini}" ]]; then
        nn_log "  No eqclient.ini found, skipping EQ resolution."
        return 0
    fi

    nn_log "Setting EQ resolution to ${res}..."

    if [[ "${DRY_RUN}" -eq 1 ]]; then
        nn_log "  [DRY-RUN] Would set Width=${width}, Height=${height}"
        return 0
    fi
    ...
    nn_log "  EQ VideoMode: ${width}x${height}"
}
...
scale_ui_positions() {
    ...
    nn_log "Scaling UI positions for ${res}..."
    ...
    for ui_file in "${eq_dir}"/UI_*_*.ini; do
        ...
        if [[ "${DRY_RUN}" -eq 1 ]]; then
            nn_log "  [DRY-RUN] Would scale positions in ${basename}"
            count=$((count + 1))
            continue
        fi
        ...
        if grep -q "XPos${width}x${height}=" "${ui_file}" 2>/dev/null; then
            nn_log "  ${basename}: ${res} positions already exist, skipping."
            count=$((count + 1))
            continue
        fi
        ...
        mv "${tmpfile}" "${ui_file}"
        nn_log "  ${basename}: scaled positions for ${res}"
        count=$((count + 1))
    done

    if [[ "${count}" -gt 0 ]]; then
        nn_log "  Scaled ${count} UI file(s)."
    fi
}
...
update_config_resolution() {
    ...
    if [[ -f "${config_file}" ]]; then
        if grep -q "^resolution:" "${config_file}" 2>/dev/null; then
            ...
            nn_log "  Updated norrath-native.yaml resolution to ${res}"
        fi
    fi
}
...
cmd_apply() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --resolution)
                if [[ $# -lt 2 ]]; then nn_log "ERROR: --resolution requires WxH"; exit 1; fi
    ...
    nn_log "Wine desktop: ${current:-unknown} → ${wine_res} (full monitor for tiling)"
    nn_log "EQ game res:  → ${eq_res} (16:9 clamped)"
    ...
    if is_ultrawide "${monitor_res}"; then
        ...
        nn_log ""
        nn_log "ULTRAWIDE: Use this viewport command in-game to center the view:"
        nn_log "  /viewport ${offset} 0 ${eq_width} ${eq_height}"
        nn_log ""
        nn_log "This gives you ${eq_res} centered game rendering with"
        nn_log "${offset}px sidebars on each side for UI elements."
    fi

    nn_log ""
    nn_log "Restart EQ for changes to take effect."
}
